...
# no threshold
    ...
    for i in range(RUN_TIMES):
        start_time = time.clock()
        plain_text = cpre_sm9.dec_1(sk_B, cp_ct_reenc, proxy_cond, 32)
        end_time = time.clock()
        run_time += end_time - start_time
    print('cpre-decrypt: %s ms' % (((run_time)*1000)/RUN_TIMES)) 
...
# (t, n) Shamir Secret Share
    ...
    for i in range(RUN_TIMES):
        start_time = time.clock()
        rk_shares = cpre_sm9.shamir_share(secret, threshold, total_shares, pk_B)
        end_time = time.clock()
        run_time += end_time - start_time
    ...
    recon_shares = [share for share in rk_shares if share[0] in recon_set]

    run_time = 0
    for i in range(RUN_TIMES):
        start_time = time.clock()
        partial_rks = []
        for share in recon_shares:
            partial_rk = cpre_sm9.shamir_recon(share, recon_set)
            partial_rks.append(partial_rk)
        end_time = time.clock()
        run_time += end_time - start_time
    print('(t,n)recon: %s ms' % (((run_time)*1000)/RUN_TIMES)) 

    # blind_rk = cpre_sm9.shamir_recon(recon_shares, pk_B)
    # rk = cpre_sm9.unblind(blind_rk, blind_key)
    # cp_ct_reenc = cpre_sm9.reenc(rk, ct_A)
    # # user B run cpre-dec to decrypt the ciphertext
    # plain_text_shamir = cpre_sm9.dec(sk_B, cp_ct_reenc, proxy_cond, 32)
